...
class GameConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def connect(self):
        ...
        with contextlib.suppress(KeyError):
            #Capacidad de la habitacion de juego: max 2 personas
            if (len(self.channel_layer.groups[self.group_name]) >=2):
                await self.accept()
                await self.send_json({
                    'event':'show error',
                    'error': 'No puedes acceder al juego. La habitacion está llena, prueba con otra'
                })
        ...
        if (len(self.channel_layer.groups[self.group_name]) ==2):
            ...
            firstPlayer=random.choice(tmpGroup)
            tmpGroup.remove(firstPlayer)
            final_group=[firstPlayer,tmpGroup[0]]
